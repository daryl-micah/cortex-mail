...
interface Message {
  ...
  steps?: AgentStep[];
  /** True when older turns were LLM-summarised to fit the token budget */
  summarised?: boolean;
  /** Tokens consumed by this turn's context (system + RAG + history) */
  budgetUsed?: number;
}
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        {
          ...
          steps: data.steps ?? [],
          summarised: data.summarised ?? false,
  ...
  return (
    <aside className="border-l border-border h-screen flex flex-col bg-background">
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar p-4 space-y-3">
        ...
        {messages.map((msg, i) => (
          <div key={i}>
            {/* Context-compressed notice — shown once when the agent had to summarise older turns */}
            {msg.role === 'assistant' && msg.summarised && (
              <div className="flex items-center gap-1.5 mr-4 mb-1">
                <div className="flex-1 h-px bg-border" />
                <span className="text-[10px] text-muted-foreground/70 whitespace-nowrap">
                  memory compressed
                </span>
                {msg.budgetUsed !== undefined && (
                  <span className="text-[10px] text-muted-foreground/50">
                    · {msg.budgetUsed.toLocaleString()} tokens
                  </span>
                )}
                <div className="flex-1 h-px bg-border" />
              </div>
